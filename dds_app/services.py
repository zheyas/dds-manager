...
def fetch_cbr_quotes(timeout=5):
    ...
    Возвращает кортеж (usd_rub, eur_rub).
    В случае ошибки возвращает (None, None).
    """
    try:
        resp = requests.get(
            "https://www.cbr-xml-daily.ru/daily_json.js", timeout=timeout
        )
...
def annotate_cashflow_with_cashback(cashflows):
    """
    Добавляет каждому объекту CashFlow
    поле cashback_floor (только для расходов).
    """
    for op in cashflows:
...
def get_cashflow_stat(user, type_, date_from, date_to):
    ...
    qs = CashFlow.objects.filter(
        user=user, type=type_, created_at__gte=date_from, created_at__lt=date_to
    )
    ...
    cashback = (
        qs.aggregate(
            cashback=Sum(
                ExpressionWrapper(
                    F("amount") * F("cashback_percent") / 100, output_field=FloatField()
                )
            )
        )["cashback"]
        or 0
