...
class CashFlowForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        elif self.initial.get('created_at') and isinstance(self.initial['created_at'], datetime.datetime):
            self.initial['created_at'] = self.initial['created_at'].strftime('%Y-%m-%dT%H:%M')
        ...
        if self.initial.get('type') == 'expense':
            ...
            self.fields['cashback_percent'].widget.attrs.update({'placeholder': '0-100', 'min': 0, 'max': 100, 'class': 'form-control'})
        else:
    ...
    class Meta:
        model = CashFlow
        fields = [
            'title', 'created_at', 'status', 'category', 'subcategory',
            'amount', 'type', 'cashback_percent', 'comment'
        ]
        widgets = {
    ...
    def clean_cashback_percent(self):
        ...
        if type_ == 'expense':
            if cashback is None:
                raise forms.ValidationError("Для трат обязательно укажите кешбэк (если его нет — укажите 0).")
    ...
    def clean(self):
        ...
        if category is None:
            ...
            if type_ and category.type != type_:
                self.add_error('category', "Тип категории и тип операции должны совпадать.")
