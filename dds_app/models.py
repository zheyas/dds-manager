...
class Category(models.Model):
    ...
    type = models.CharField(
        max_length=10, choices=TYPE_CHOICES, verbose_name="Тип операции"
    )
...
class CashFlow(models.Model):
        ...
        if self.type == "expense":
            if self.cashback_percent is None:
                ...
                errors["cashback_percent"] = (
                    "Кешбэк должен быть в диапазоне от 0 до 100."
                )
        else:
    ...
    def cashback_amount(self):
        ...
        if self.type != "expense" or not self.cashback_percent:
            return Decimal("0.00")
        value = (self.amount * self.cashback_percent / 100).quantize(
